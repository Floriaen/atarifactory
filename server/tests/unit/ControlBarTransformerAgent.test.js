...
describe('ControlBarTransformerChain', () => {
  it('converts keyboard/mouse input to control bar events', async () => {
    ...
    const mockLLM = new MockLLM(transformed);
  ...
  it('injects minimal handlers if none present', async () => {
    ...
    const minimalMockLLM = new MockLLM(transformedMinimal);
  ...
  it('leaves unrelated code untouched', async () => {
    ...
    const mockLLM = new MockLLM(code);
    const sharedState = { gameSource: code };
    const out = await transformGameCodeWithLLM(sharedState, mockLLM);
  ...
  it('transforms a real generated game (fixture) to use control bar only', async () => {
    ...
    const transformedGame = gameSource
    ...
    const mockLLM = new MockLLM(transformedGame);
  ...
  const runManualLLMTest = process.env.RUN_MANUAL_CONTROL_BAR_TEST === '1';

  (runManualLLMTest ? it : it.skip)('real LLM transforms real generated game (manual run)', async () => {
